...
flags.DEFINE_integer('netperf_max_iterations', None,
...
flags.DEFINE_integer('netperf_test_len', 60,
                     'netperf test length, in seconds',
                     lower_bound=1)
flags.DEFINE_bool('netperf_enable_hist', True,
                  'Determines whether latency histograms are '
                  'collected/reported. Only for *RR benchmarks')
flag_util.DEFINE_integerlist('netperf_num_stream', flag_util.IntegerList([1]),
                             'Number of netperf processes to run. Netperf '
                             'will run once for each value in the list.')
flags.DEFINE_integer('netperf_think_time', 0,
                     'Time in nanoseconds to do work for each request.')
flags.DEFINE_integer('netperf_think_time_array_size', 0,
                     'The size of the array to traverse for thinktime.')
flags.DEFINE_integer('netperf_think_time_run_length', 0,
...
flags.DEFINE_list('netperf_benchmarks_to_run', ALL_BENCHMARKS,
                  'The netperf benchmark(s) to run.')
flags.register_validator(
    'netperf_benchmarks_to_run',
    lambda benchmarks: benchmarks and set(benchmarks).issubset(ALL_BENCHMARKS))

FLAGS = flags.FLAGS

BENCHMARK_NAME = 'local_to_cloud'
BENCHMARK_CONFIG = """
local_to_cloud:
...
def Prepare(benchmark_spec):
  ...
  #logging.info('This is the PREPARE PART')
  print(benchmark_spec)
  #for attr in dir(benchmark_spec):
  #  print("obj.%s = %r" % (attr, getattr(benchmark_spec, attr)))

  vms = benchmark_spec.vms
  vms = vms[:2]
  vm_util.RunThreaded(PrepareNetperf, vms)

  num_streams = max(FLAGS.netperf_num_stream)
...
def RunNetperf(vm, benchmark_name, server_ip, num_streams):
  ...
  """
  enable_latency_histograms = FLAGS.netperf_enable_hist or num_streams > 1
  ...
  confidence = ('-I 99,5 -i {0},3'.format(FLAGS.netperf_max_iterations)
                if FLAGS.netperf_max_iterations else '')
  ...
  netperf_cmd = ('{netperf_path} -p {{command_port}} -j {verbosity} '
                 ...
                 'CONFIDENCE_ITERATION,THROUGHPUT_CONFID').format(
                     ...
                     server_ip=server_ip,
                     length=FLAGS.netperf_test_len,
                     confidence=confidence, verbosity=verbosity)
  if FLAGS.netperf_think_time != 0:
    netperf_cmd += (' -X {thinktime},{thinktime_array_size},'
                    '{thinktime_run_length} ').format(
                        thinktime=FLAGS.netperf_think_time,
                        thinktime_array_size=FLAGS.netperf_think_time_array_size,
                        thinktime_run_length=FLAGS.netperf_think_time_run_length)
  ...
  # complete
  remote_cmd_timeout = \
      FLAGS.netperf_test_len * (FLAGS.netperf_max_iterations or 1) + 300
  ...
  metadata = {'netperf_test_length': FLAGS.netperf_test_len,
              'max_iter': FLAGS.netperf_max_iterations or 1,
...
def Run(benchmark_spec):
  ...
  for num_streams in FLAGS.netperf_num_stream:
    assert(num_streams >= 1)

    for netperf_benchmark in FLAGS.netperf_benchmarks_to_run:
      if vm_util.ShouldRunOnExternalIpAddress():
